...
const int publishPeriod = 10000;
int publishTime = 0;
char* topic = "SensorCentral";
volatile bool console = false;
...
void setup() {
  ...
  Serial.print(LCD05_I2C_ADDRESS, HEX);
  Serial.println(" ...");
  ...
  publishTime = millis() + publishPeriod;
}
...
void loop() {
  ...
  if (alarm) {
    ...
    if (publishTime <= millis()) {
      openConsole();
      char topublish[25] = "";
      sprintf(topublish, "%d:%d -> %d - %dcm", rtc.getHours(), rtc.getMinutes(), personas, altura);
      publishToTopic(topublish, topic);
      closeConsole();
      publishTime = millis() + publishPeriod;
    }
...
void publishToTopic(char* message, char* topic) {
  Keyboard.print("mosquitto_pub -h localhost -m ");
  Keyboard.print('"');
  Keyboard.print(message);
  Keyboard.print('"');
  Keyboard.print(" -t ");
  Keyboard.println(topic);
}
